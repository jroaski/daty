...
class FunctionsTests(unittest.TestCase):
    ...
    def test_valid_date(self, line, expected):
        ...
        result = datetime.strptime(result_str, "%d.%m.%Y") if result_str else None  # Convert the string to a datetime object
        self.assertEqual(result, expected)

    @parameterized.expand([
        (["01.08.2023 radom shrandom words", "Text 2"], [["01.08.2023 radom shrandom words", "Text 2"]]),
        (["Text 2"], []),
    ])
    def test_lines_by_dates(self, lines, expected):
        date_line_dict = group_lines_by_dates(lines)
        actual = [value for value in date_line_dict.values()] if date_line_dict else []
